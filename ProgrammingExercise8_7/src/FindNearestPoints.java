...
public class FindNearestPoints {
  public static void main(String[] args) {
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      // 3 dimensions - added 3rd
      points[i][2] = input.nextDouble();
    }

    // p1 and p2 are the indices in the points array
    int p1 = 0, p2 = 1, p3 = 2; // Initial THREE points
    double shortestDistance = distance(points[p1][0], points[p1][1], points[p1][2], points[p2][0], points[p2][1], points[p3][p2]); // Initialize
                                                                                                    // shortestDistance

    // Compute distance for every two points
    for (int i = 0; i < points.length; i++) {
      for (int j = i + 1; j < points.length; j++) {
        double distance = distance(points[i][0], points[i][1], points[i][2], points[j][0], points[j][1], points[j][2]); // Find
                                                                                            // distance
